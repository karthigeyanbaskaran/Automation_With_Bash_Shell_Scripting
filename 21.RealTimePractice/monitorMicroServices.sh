...
if [[  ${httpStatusCode} -eq 200 ]] ; then 
    ...
    ${ECHO} "The Micro-Service with helath url : ${healthURL} is not up and running" | ${TEE} -a ${LOG_FILE}
    ${ECHO} "The Micro-Service with helath url : ${healthURL} is not up and running" | ${MAILX} -s "Alert: Micro-Service Status" ${mailTo}
fi